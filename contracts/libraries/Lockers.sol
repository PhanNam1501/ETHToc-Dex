...
/// struct LockData {
    ...
    function push(address locker, address lockerCaller) internal {
        assembly {
            ...
            tstore(add(indexToWrite, 1), lockerCaller)
    ...
    function getLockerCaller(uint256 i) internal view returns (address locker) {
        assembly {
    ...
    function getCurrentLockCaller() internal view returns (address) {
        unchecked {
            uint256 l = length();
            return l > 0 ? getLockerCaller(l - 2) : address(0);
        }
    }
